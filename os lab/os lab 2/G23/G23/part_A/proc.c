...
//remove corner cases -- extra lock for pqueue
void pinit(void)
{
...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->nocs = 0;		//initialize nocs
  p->parent = &defaultParent; // initialize parent to default parent
  p->burst_time = 0;   //initialize burst_time
  p->rt = 0;		//running time initialize to 0
  release(&ptable.lock);
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  //Insert Process Into Queue while checking
  if(check)
  	insertIntoHeap(np);
